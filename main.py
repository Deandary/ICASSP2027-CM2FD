...
def main(args):
    ...
    if args.mode == "train":
        ...
        if enable_phase1:
            ...
            for current_epoch in range(0, args.stage1_epoch):
                ...
                cmc, mAP, mINP = test(args, model, dataset,enable_phase1=enable_phase1)
                ...
                # model.update_scheduler(phase='phase1', epoch=current_epoch, metric=mAP)
        ...
        for current_epoch in range(start_epoch, args.stage2_epoch):
            ...
            cmc, mAP, mINP = test(args, model, dataset, enable_phase1=enable_phase1)
            # 2. 新增：分别判断三个指标是否达到最优
            is_best_rank1 = (cmc[0] >= best_rank1)  # Rank-1最优标记
            is_best_mAP = (mAP >= best_mAP)  # mAP最优标记
            is_best_mINP = (mINP >= best_mINP)  # mINP最优标记（新增）

            # 更新三个指标的最优值和对应epoch
            if is_best_rank1:
                best_rank1 = cmc[0]
                best_rank1_epoch = current_epoch + 1
            if is_best_mAP:
                best_mAP = mAP
                best_mAP_epoch = current_epoch + 1
            if is_best_mINP:
                best_mINP = mINP
                best_mINP_epoch = current_epoch + 1

            # 3. 修改：调用保存模型方法，传入三个最优标记（替代原来仅传入rank1最优标记）
            model.save_model(current_epoch, is_best_rank1, is_best_mAP, is_best_mINP)
            logger('=================================================\nEpoch: {};Time: {};Setting: {}' \
                   .format(current_epoch, time_now(), args.save_path))
            logger(f'e_lr: {model.scheduler_phase2.get_lr()[0]}')
            logger(result)
            logger('R1:{:.4f};   R10:{:.4f};  R20:{:.4f};  mAP:{:.4f};  mINP:{:.4f}\n\
                               Best_R1: {:.4f}(epoch:{});    Best mAP: {:.4f}(epoch:{});    Best mINP: {:.4f}(epoch:{})' \
                   .format(cmc[0], cmc[9], cmc[19], mAP, mINP,
                           best_rank1, best_rank1_epoch,
                           best_mAP, best_mAP_epoch,
                           best_mINP, best_mINP_epoch))
            logger('=================================================')
            # model.update_scheduler(phase='phase2', epoch=current_epoch, metric=mAP)
    ...
    if args.mode == 'test':
        ...
        if args.model_path == 'default':
            ...
            model.resume_model(args.model_path)
        cmc, mAP, mINP = test(args, model, dataset, enable_phase1=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", default="/home/share/qsdeng/datasets/", type=str, help="dataset path")
    ...
    parser.add_argument('--conf_filter_thresh', type=float, default=0.3,
                        help='Base confidence threshold for common samples filtering (will sync with dynamic growth)')

    parser.add_argument('--diffusion_weight', type=float, default=0.25, help='Weight for feature diffusion loss')
    parser.add_argument('--sigma', type=float, default=0.8, help='Momentum factor for CMA memory bank update')
    parser.add_argument('--cross_weight', type=float, default=1.0,
                        help='Weight for cross-modality loss of high-conf specific samples')
    parser.add_argument('--weak_weight', type=float, default=0.25,
                        help='Weight for weak supervision loss of low-conf samples')
    parser.add_argument('--tri_weight', type=float, default=1,
                        help='Weight for Triplet loss of high-conf common samples')
    parser.add_argument('--diffusion_timesteps', type=int, default=12, help='Number of timesteps for feature diffusion')

    parser.add_argument('--milestones', nargs='+', type=int, default=[30, 100],
    ...
    parser.add_argument("--device", default=1, type=int, help="gpu")
    parser.add_argument('--stage1-epoch', default=20, type=int, help='s:20, l:80, r:50')
    parser.add_argument('--stage2-epoch', default=150, type=int, help='CMCL total epoch')
    ...
    # 新增：动态衰减相关参数（可选）
    parser.add_argument('--dynamic-decay-patience', default=3, type=int, help='Patience for dynamic LR decay')
    parser.add_argument('--gradual-decay-rate', default=0.9, type=float, help='Gradual decay rate after warmup')

    args = parser.parse_args()
    args.save_path = './log/saved_'+args.dataset+'_{}'.format(args.arch)+'/'+args.save_path
