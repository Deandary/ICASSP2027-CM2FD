...
class Two_dimensional_Fast_Fourier_Transform(nn.Module):
    ...
    def forward(self, x):
        assert len(x.shape) == 4, f"Input must be 4D (B×C×H×W), got {len(x.shape)}D"
        B, C, H, W = x.shape
        x_flatten = x.view(B, C, H * W).contiguous()
        # 频率滤波（强制CPU执行，避免GPU cuFFT错误）
        fts_low, fts_mid, fts_high = self.filter_frequency_bands(x_flatten, cutoff=0.30)
        # 还原为4维并移回原设备（GPU）
    ...
    def reshape_to_square(self, tensor):
        ...
        side_length += 1 if side_length % 2 != 0 else 0
        ...
        return padded_tensor.contiguous().view(B, C, side_length, side_length).contiguous(), side_length, side_length, N

    def filter_frequency_bands(self, tensor, cutoff=0.2):
        # 核心修改：强制所有FFT相关操作在CPU执行
        tensor_cpu = tensor.cpu().to(dtype=torch.float32)
        original_device = tensor.device  # 记录原始设备（GPU）

        # 过滤异常值
        if torch.isnan(tensor_cpu).any() or torch.isinf(tensor_cpu).any():
            tensor_cpu = torch.nan_to_num(tensor_cpu, nan=0.0, posinf=1e5, neginf=-1e5)

        # 重塑为方形张量（CPU上执行）
        tensor_sq, H, W, N = self.reshape_to_square(tensor_cpu)
        B, C, _, _ = tensor_sq.shape
        ...
        # CPU生成掩码（彻底避免GPU内存问题）
        def create_filter(shape, low_cutoff, high_cutoff, mode='band'):
            rows, cols = shape
            ...
            return torch.from_numpy(mask).unsqueeze(0).unsqueeze(0).repeat(B, C, 1, 1)

        low_pass = create_filter((H, W), low_cutoff, None, 'low')
        mid_pass = create_filter((H, W), low_cutoff, high_cutoff, 'band')
        high_pass = create_filter((H, W), None, high_cutoff, 'high')

        # 关键：所有FFT操作在CPU执行（无cuFFT依赖）
        try:
            # 正向FFT（CPU）
            fft_tensor = torch.fft.fft2(tensor_sq, dim=(-2, -1))
            fft_shifted = torch.fft.fftshift(fft_tensor, dim=(-2, -1))

            # 应用掩码
            fft_low = fft_shifted * low_pass
            fft_mid = fft_shifted * mid_pass
            fft_high = fft_shifted * high_pass

            # 逆FFT（CPU），取实部
            low_tensor = torch.fft.ifft2(torch.fft.ifftshift(fft_low, dim=(-2, -1)), dim=(-2, -1)).real
            mid_tensor = torch.fft.ifft2(torch.fft.ifftshift(fft_mid, dim=(-2, -1)), dim=(-2, -1)).real
            high_tensor = torch.fft.ifft2(torch.fft.ifftshift(fft_high, dim=(-2, -1)), dim=(-2, -1)).real

            # 裁剪回原始长度，移回原设备（GPU）
            low_tensor = low_tensor.view(B, C, -1)[:, :, :N].contiguous().to(original_device)
            mid_tensor = mid_tensor.view(B, C, -1)[:, :, :N].contiguous().to(original_device)
            high_tensor = high_tensor.view(B, C, -1)[:, :, :N].contiguous().to(original_device)

        except Exception as e:
            print(f"CPU FFT Error: B={B}, C={C}, H={H}, W={W}, device={original_device}")
            raise e
...
class Multi_FUE(nn.Module):
    def __init__(self, channel=512, dim=128,reduction=16, HFF_dp=0.,group_ratio=1,num_heads=16,window_size=3,shift_size=1,mlp_ratio=4):
        ...
        self.feature_enhancer = nn.Sequential(
            # nn.Conv2d(ch_int, ch_int, kernel_size=3, padding=1, groups=ch_int),
            WTConv2d(1024, 1024, kernel_size=3, stride=1, bias=False, wt_levels=1, wt_type='db1'),
            nn.BatchNorm2d(1024),
            nn.GELU(),
            nn.Conv2d(1024, 1024, kernel_size=1),
            nn.BatchNorm2d(1024)
        )
        self.block = Two_dimensional_Fast_Fourier_Transform()
        self.MSFM = MSFM(feature_dim=1024)
        self.dropout = nn.Dropout(p=0.01)

    def forward(self, x):
        x21, x22, x23 = self.block(x)
        B, C, H, W = x.shape
        x2 = self.MSFM(x21, x22, x23)
        x2 = x2.view(B, C, H, W).contiguous()
        out =  x2
        return out + self.feature_enhancer(x)
...
class FAMN(nn.Module):
    def __init__(self, args, arch='resnet50', planes=64):
        super(FAMN, self).__init__()
